...
This plan outlines the steps required to convert the backend from **Flask** to **FastAPI**, enable **database table auto-generation**, and deploy the server to **Render over HTTPS (SSL/TLS)** for secure data transfer to your React web app.
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allows HTTPS requests from React Web and local apps
...
ngrok http 5000
```
It will output a URL like `https://xxxx-xx-xx-xx.ngrok-free.app` which you can paste into your client `.env` file.
...
- [ ] Update frontend environment variables in the Web app to target the `https://` Render backend URL.
